...
export function patchWorldV11Source(source, assetBase){
  ...
  let patched=source.replace("shapeVersion:'fantasy-rts-v1'","shapeVersion:'readable-world-v2'");
  patched=patched.replace(mapPattern,`${readableMapBlock}\nclass WorldSimulation extends EventTarget {`);
  patched=patched.replace(terrainPattern,readableTerrainMethod);
  patched=patched.replace("background:'#102d4c'","background:'#12445f'");
  const base=String(assetBase||'');
  if(base){patched=patched.replaceAll("import('./",`import('${base}`);patched=patched.replaceAll('import("./',`import("${base}`);}
  return patched;
}
