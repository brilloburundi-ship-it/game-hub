(() => {
'use strict';
const VERSION = 'v71-surgical-gameplay-fixes-1';
const SHIELD_SECONDS = 120;
const FARMER_SPEED_FACTOR = 0.58;
const WALK_ANIM_SPEED = 0.085;
const WORK_ANIM_SPEED = 0.095;
const MAX_VISIBLE_WAR_TOTAL = 22;
const MAX_EXTRA_WAR_UNITS_GLOBAL = 72;
...
return (sim.kingdoms || []).find(k => (k.buildings || []).includes(building)) ||
sim.kingdoms?.[building.owner] || null;
}
...
function desiredVisibleWarTotal(sim, kingdom) {
const power = Math.max(0, Number(sim.power?.(kingdom)) || 0);
return clamp(Math.round(5 + Math.sqrt(power) * 0.32), 6, MAX_VISIBLE_WAR_TOTAL);
}
...
function updatePowerArmyVisuals(sim, renderer, state, dt) {
if (!renderer.entities || typeof renderer.makeSoldier !== 'function') return;
const activeWars = (sim.wars || []).filter(w => !w.done);
const activeKeys = new Set();
let globalExtras = 0;
for (const arr of state.powerArmyVisuals.values()) globalExtras += arr.length;
for (const war of activeWars) {
if (!war.front) continue;
for (const side of [war.a, war.b]) {
const kingdom = sim.kingdoms?.[side];
if (!kingdom?.alive) continue;
const key = `${war.id}:${side}`;
activeKeys.add(key);
const actual = (renderer.__v66Guards?.get?.(side) || []).filter(u => !u.dead && (!u.warId || u.warId === war.id)).length;
const desiredTotal = desiredVisibleWarTotal(sim, kingdom);
let desiredExtra = Math.max(0, desiredTotal - actual);
desiredExtra = Math.min(desiredExtra, Math.max(0, MAX_EXTRA_WAR_UNITS_GLOBAL - (globalExtras - (state.powerArmyVisuals.get(key)?.length || 0))));
const arr = state.powerArmyVisuals.get(key) || [];
while (arr.length < desiredExtra) {
const i = arr.length;
const role = i % 5 === 4 ? 'archer' : 'sword';
const sprite = renderer.makeSoldier(kingdom, role);
if (!sprite) break;
stripSpearGraphic(sprite);
sprite.scale.set(0.82);
sprite.eventMode = 'none';
renderer.entities.addChild(sprite);
arr.push({ s: sprite, role, phase: Math.random() * 6 });
globalExtras++;
}
while (arr.length > desiredExtra) {
destroyDisplay(arr.pop()?.s);
globalExtras--;
}
state.powerArmyVisuals.set(key, arr);
const ownCell = side === war.a ? war.front[0] : war.front[1];
const enemyCell = side === war.a ? war.front[1] : war.front[0];
const own = sim.iso(...ownCell), enemy = sim.iso(...enemyCell);
let dx = enemy[0] - own[0], dy = enemy[1] - own[1];
const len = Math.max(1, Math.hypot(dx, dy)); dx /= len; dy /= len;
const px = -dy, py = dx;
const phase = war.__v66?.phase || 'advance';
const cols = Math.min(6, Math.max(3, Math.ceil(Math.sqrt(Math.max(1, arr.length)) * 1.8)));
for (let i = 0; i < arr.length; i++) {
const unit = arr[i];
if (!unit?.s || unit.s.destroyed) continue;
unit.phase += dt * 4;
const row = Math.floor(i / cols), col = i % cols;
const lane = (col - (cols - 1) / 2) * 5.3;
const depth = 23 + row * 7 + (unit.role === 'archer' ? 7 : 0);
const x = own[0] - dx * depth + px * lane;
const y = own[1] - dy * depth + py * lane + 5;
unit.s.position.set(x, y);
unit.s.zIndex = Math.round(y * 100) + 15;
if (unit.s._sprite) {
const mag = Math.abs(unit.s._sprite.scale.x || 1);
unit.s._sprite.scale.x = enemy[0] >= x ? mag : -mag;
}
const mode = phase === 'combat' ? (Math.sin(unit.phase + i) > 0.15 ? 'attack' : 'idle') : 'walk';
renderer.swapAnim?.(unit.s, mode);
}
}
}
for (const [key, arr] of [...state.powerArmyVisuals]) {
if (activeKeys.has(key)) continue;
for (const unit of arr) destroyDisplay(unit?.s);
state.powerArmyVisuals.delete(key);
}
state.armySortClock = (state.armySortClock || 0) + dt;
if (renderer.entities?.sortableChildren && state.armySortClock >= 0.14) {
state.armySortClock = 0;
...
if (sim?.r?.app?.ticker && window.__V70_WAR_PEACE_CLEANUP?.installed &&
window.__V67_PIXEL_BUILDINGS?.installed && sim.__v661BattleStabilityInstalled) break;
await sleep(20);
}
...
if (kingdom?.alive && isAtWar(this, kingdom)) {
return applyWarGift(this, renderer, kingdom, giftName, repeat, meta);
}
...
let slowClock = 0;
renderer.app.ticker.add(function v71SurgicalRuntime(ticker) {
const dt = clamp(Number(ticker?.deltaMS || renderer.app.ticker.deltaMS || 16.7) / 1000, 0.001, 0.05);
updateConfirmedFires(sim, renderer, dt);
updatePowerArmyVisuals(sim, renderer, state, dt);
slowClock += dt;
if (slowClock < 0.35) return;
slowClock = 0;
...
visibleArmyScalesWithPower: true
});
