(() => {
  ...
  class PixiRenderer {
    ...
    async init(sim) {
      ...
      await this.app.init({ resizeTo: window, background: '#153f61', antialias: false, autoDensity: true, resolution: Math.min(window.devicePixelRatio || 1, 1.5), preference: 'webgl', powerPreference: 'high-performance' });
    ...
    async addFarmer(k, f) {
      const s = this.makeFarmerSprite('idle'); this.applyFarmerScale(s, 'idle'); s.position.set(f.x, f.y); s.zIndex = Math.round(f.y * 100) + 10; this.entities.addChild(s); f._sprite = s; this.farmerSprites.set(f.id, s);
    }
    removeFarmer(f) {
    ...
    updateFarmer(f, dx, dy) {
      ...
      s.position.set(f.x, f.y); s.zIndex = Math.round(f.y * 100) + 10; this.entities.sortDirty = true;
    }
