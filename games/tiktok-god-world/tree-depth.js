(() => {
  ...
  const install = async () => {
    ...
    for (const tree of data.trees) {
      ...
      const key = cellKey(tree.cell[0], tree.cell[1]);
    ...
    window.__TREE_DEPTH_READY = { count: data.trees.length, version: data.version };
    document.documentElement.dataset.treeDepth = `${data.version}:${data.trees.length}`;
  };
