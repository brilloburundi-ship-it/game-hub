(() => {
  ...
  const MAX_WORLD_TREES = 150;
  const MIN_CELL_GAP = 1.65;
  ...
  function chooseSparseTrees(data, sim) {
    ...
    if (selected.length < Math.min(110, MAX_WORLD_TREES)) {
  ...
  const install = async () => {
    ...
    for (const tree of chosen) {
      ...
      const h = hashCell(tree.cell[0], tree.cell[1]);
      const scale = 0.60 + (h % 18) / 100;
    ...
    renderer.prepareBuildSite = async (kingdom, x, y, battleSim = sim) => {
      ...
      if (worker) {
        ...
        worker.actionUntil = battleSim.age + 0.4;
      ...
      for (const sprite of trees) {
        ...
        for (let i = 0; i < 6; i++) chips.rect(Math.random() * 8 - 4, Math.random() * 7 - 5, 2, 2).fill({ color: i % 2 ? 0x8d5b31 : 0xd59b53, alpha: .9 });
    ...
    window.__TREE_DEPTH_READY = { count: chosen.length, sourceCount: (data.trees || []).length, version: 'sparse-v2' };
    document.documentElement.dataset.treeDepth = `sparse-v2:${chosen.length}`;
  };
