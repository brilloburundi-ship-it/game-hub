  ...
  function install(sim) {
    ...
    sim.addBuilding = async function (...args) {
      ...
      if (building) {
        ...
        // Construction growth animation can briefly touch the base graphics,
        // so enforce again after it has finished without adding a frame loop.
        requestAnimationFrame(() => removeFloatingBase(building, this.r));
        setTimeout(() => removeFloatingBase(building, this.r), 1650);
      }
      return building;
    };

    // One low-frequency guard is enough for old cached/building-animation edge cases.
    setInterval(() => enforceKingdoms(sim), 1800);
  }
