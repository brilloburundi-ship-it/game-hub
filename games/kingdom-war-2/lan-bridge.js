(() => {
  ...
  if (suppliedToken) localStorage.setItem('kingdom_war_2_bridge_token', suppliedToken);
  const token = suppliedToken || localStorage.getItem('kingdom_war_2_bridge_token') || '';
  const queuedEvents = [];
  ...
  function deliver(payload) {
    if (payload?.__bridgeStatus) {
      ...
      else setStatus('PC online Â· waiting for TikFinity', '#d39d34');
      return;
    }
  ...
  function connect() {
    ...
    setStatus('connecting to PCâ€¦', '#d39d34');
    const stream = new EventSource(`/bridge/events?token=${encodeURIComponent(token)}`);
    stream.onopen = () => setStatus('PC online Â· waiting for TikFinity', '#d39d34');
    stream.onmessage = message => {
      try { deliver(JSON.parse(message.data)); }
      catch (error) { console.warn('[Kingdom War 2 LAN] Event ignored', error); }
    };
