...
const PORT = Number(process.env.KINGDOM_WAR_2_PORT || 8794);
const HOST = '0.0.0.0';
const TIKFINITY_URL = process.env.TIKFINITY_URL || 'ws://127.0.0.1:21213/';
const APP_ID = 'kingdom-war-2';
const BUNDLED_MUSIC = join(ROOT, 'assets', 'audio', 'medieval-market-full.mp3');
const DOWNLOAD_MUSIC = join(homedir(), 'Downloads', 'Medieval Fantasy Music â€“ Medieval Market _ Folk, Traditional, Instrumental _ Fantasy Music World #2.mp3');
const MUSIC_FILE = process.env.KINGDOM_WAR_2_MUSIC || (existsSync(BUNDLED_MUSIC) ? BUNDLED_MUSIC : DOWNLOAD_MUSIC);
const MAX_EVENT_BYTES = 256 * 1024;
const MAX_CLIENTS = 8;
const TOKEN_DIR = process.env.LOCALAPPDATA
  ? join(process.env.LOCALAPPDATA, 'KingdomWar2Bridge')
  : join(homedir(), '.kingdom-war-2-bridge');
...
server.on('error', error => {
  if (error?.code === 'EADDRINUSE') {
    ...
    console.error('Se Kingdom War 2 e gia aperto, usa lo stesso URL su Safari.');
...
server.listen(PORT, HOST, () => {
  ...
  if (process.env.KINGDOM_WAR_2_SKIP_URL_FILE !== '1') writeFileSync(join(ROOT, 'URL_IPHONE.txt'), output, 'utf8');

  console.log('\n=============================================================');
  console.log(' KINGDOM WAR 2 - SAFARI + TIKFINITY (8794)');
  console.log('=============================================================');
  ...
  connectTikFinity();
  if (process.env.KINGDOM_WAR_2_NO_OPEN !== '1') setTimeout(openLocalUrl, 500);
});
