(() => {
  ...
  let minifolksManifestServed = false;
  ...
  window.fetch = async function(input, init) {
    ...
    if (url.endsWith('assets/minifolks/manifest.json')) {
      const response = await nativeFetch(input, init);
      if (!response.ok) return response;
      minifolksManifestServed = true;
      return jsonResponse(response, await response.clone().json());
    }
  ...
  async function installRuntimePatches() {
    ...
    if (typeof renderer.makeFarmerSprite === 'function') {
      ...
      renderer.makeFarmerSprite = function(...args) {
        const action = args[0];
        const sprite = originalMakeFarmerSprite(...args);
