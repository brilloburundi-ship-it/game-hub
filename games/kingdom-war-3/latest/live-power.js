(() => {
  ...
  function strengthenGuards(sim,r){const map=r?.__v66Guards;if(!(map instanceof Map))return;for(const[id,list]of map){const k=sim.kingdoms?.[id];if(!k?.alive)continue;const mult=combatMult(sim,k);k.__v713CombatMultiplier=mult;for(const u of list||[]){if(!u||u.dead)continue;const base=baseHp(u),next=base*mult,old=Math.max(base,Number(u.__v713PowerMaxHp||base));let hp=Number.isFinite(u.__v661Hp)?u.__v661Hp:base;if(next>old)hp+=next-old;else if(next<old)hp=Math.min(hp,next);u.__v661Hp=clamp(hp,0,next);u.__v713PowerMaxHp=next;}}}
  function speedHits(sim,r){const map=r?.__v66Guards;if(!(map instanceof Map))return;const now=performance.now();for(const[id,list]of map){const k=sim.kingdoms?.[id];if(!k?.alive)continue;const mult=combatMult(sim,k);for(const u of list||[]){if(!u||u.dead||u.state!=='combat'||!Number.isFinite(u.__v661NextHit))continue;if(u.__v713SeenNextHit===u.__v661NextHit)continue;u.__v661NextHit=now+Math.max(0,u.__v661NextHit-now)/mult;u.__v713SeenNextHit=u.__v661NextHit;}}}
  async function install(){for(let i=0;i<2200;i++){const s=window.__SIM;if(s?.r&&window.__V712_ENGAGEMENT_RECOVERY?.installed&&s.__v661BattleStabilityInstalled&&typeof s.power==='function')break;await sleep(20);}const sim=window.__SIM;if(!sim?.r)throw new Error('simulation unavailable');
  ...
  install().catch(e=>{state.errors.push(String(e?.message||e));console.error('[v713-live-power]',e);});
})();
