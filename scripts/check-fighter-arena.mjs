...
assert(index.includes(`game.js?v=${version.version}`),'index.html game cache-bust does not match release version');
assert(game.includes(`const VERSION='${version.version}'`),'game.js version does not match version.json');
assert(manifest.arenas.length===6,'Expected exactly six Fighter Arena scenes');
...
assert(combat.includes('drawFallbackFighter'),'Safe animated fighter fallback missing');
assert(!game.includes('chooseFallbackAtlas'),'Cross-atlas fallback must never return');
...
for(const[id,f]of Object.entries(fighters)){
  ...
  for(const[name,a]of Object.entries(f.animations||{})){
    ...
    assert(Number.isFinite(a.frames)&&a.frames>0,`${id}/${name}: invalid frame count`);
    assert(Number.isFinite(a.fps)&&a.fps>0,`${id}/${name}: invalid fps`);
  }
}
for(const name of ['game.js','core.js','combat.js']){
  const p=resolve(root,name);const r=spawnSync(process.execPath,['--check',p],{encoding:'utf8'});
  assert(r.status===0,`${name} syntax check failed: ${r.stderr||r.stdout}`);
}
console.log(`Fighter Arena ${version.version} checks passed: ${Object.keys(fighters).length} fighters, ${manifest.arenas.length} HD arenas.`);
