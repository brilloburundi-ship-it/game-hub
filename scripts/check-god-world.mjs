...
const [index, sw, versionText, treeDepth, living, battle, music, bootstrap, environment, game, packageJson] = await Promise.all([
  ...
  read('music.js'),
  read('world-bootstrap.js'),
  read('world-environment.js'),
  read('game.js'),
...
// Protect the proven stable core byte-for-byte. Git blob SHA = SHA1("blob <size>\\0" + content).
...
if (gameBlobSha !== '8c6b2fd651077a86e2622747a372d0d416eaabcf') {
  throw new Error(`Stable game.js core changed unexpectedly: ${gameBlobSha}`);
}
...
if (!(index.indexOf('world-environment.js') < index.indexOf('v66-living-battles.js'))) throw new Error('World environment must finish installing before battle support');
...
if (!bootstrap.includes('function chooseLakes')) throw new Error('Interactive lake generation missing');
...
for (const virtualAsset of ['assets/map/world.json', 'assets/map/vegetation.json', 'assets/map/world.png']) {
  if (!bootstrap.includes(virtualAsset.replaceAll('/', '\\/')) && !bootstrap.includes(virtualAsset)) throw new Error(`World bootstrap interception missing for ${virtualAsset}`);
}
...
if (!environment.includes('sim.lakeSet')) throw new Error('Lake collision data missing');
if (!environment.includes('sim.nearFreshWater')) throw new Error('Fresh-water adjacency logic missing');
if (!environment.includes('freshFarms')) throw new Error('Fresh-water farm bonus missing');
if (!environment.includes('sim.__gwStarterJoinInstalled')) throw new Error('Starter village JOIN wrapper missing');
if (!environment.includes('seedStarterVillage')) throw new Error('Starter village seeding missing');
...
// Execute the world generator in a DOM-light VM and validate the generated data, not just source markers.
const sandbox = {
  console,
  Response,
  setTimeout,
  clearTimeout,
  window: {
    fetch: async () => { throw new Error('Unexpected native fetch in world bootstrap test'); },
    PIXI: { Assets: { load: async () => ({}) }, Texture: { from: value => value } }
  },
  document: {
    documentElement: { dataset: {} },
    createElement: () => { throw new Error('Canvas rendering should not be needed for world-data validation'); }
  }
};
...
if (!generated) throw new Error('World bootstrap did not expose generated world data');
if (generated.gridW !== 88 || generated.gridH !== 64) throw new Error(`Unexpected generated world size ${generated.gridW}x${generated.gridH}`);
if (generated.mapWidth !== 4000 || generated.mapHeight !== 1900) throw new Error('Unexpected generated map dimensions');
...
if (!Array.isArray(generated.lakes) || generated.lakes.length < 40) throw new Error(`Not enough interactive lake cells: ${generated.lakes?.length || 0}`);
if (!Array.isArray(generated.rivers) || generated.rivers.length < 3 || generated.rivers.some(path => path.length < 8)) throw new Error('Generated rivers are missing or too short');
...
for (const [x, y] of [[9,10],[79,12],[78,56],[15,56]]) if (!generated.land?.[y]?.[x]) throw new Error(`Expected small island land missing near ${x},${y}`);
...
for (const roleFile of ['world-bootstrap.js', 'world-environment.js']) if (!shellEntries.includes(roleFile)) throw new Error(`${roleFile} must be cached by the PWA`);
...
console.log(`TikTok God World stable-integrated-1 OK: core ${gameBlobSha}, ${generated.gridW}x${generated.gridH} world, ${generated.lakes.length} lake cells, ${generated.rivers.length} rivers, ${vegetation.trees.length} source trees, starter village + neutral civilians, V6.6 safe battle frame`);
